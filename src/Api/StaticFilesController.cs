...
public class StaticFilesController : ControllerBase
{
    ...
    {
        // The HTML uses relative asset paths ("assets/app.css") so the document
        // base URL must end in "/Jellyflix/Web/". If the client requested
        // "/Jellyflix/Web" (no trailing slash), the browser would resolve assets
        // against "/Jellyflix/" and 404. Redirect to the canonical URL.
        var path = Request.Path.Value ?? string.Empty;
        if (path.EndsWith("/Web", System.StringComparison.OrdinalIgnoreCase))
        {
            return Redirect(path + "/" + Request.QueryString);
        }
        return Serve("Jellyfin.Plugin.Jellyflix.web.index.html", "text/html; charset=utf-8");
    }
